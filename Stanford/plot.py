...
def histogram(data, xlabel = "", bin_nr = 10):
...
def hist_3D(position): 
	fig = plt.figure()
	ax = fig.add_subplot(111, projection='3d')
	hist, xedges, yedges = np.histogram2d(position[:,0], position[:,2], bins=(20,20))
...
def event_display(numPhotons, photon_track, vertex, o_center, v_center, BIloc):
	...
	ax.plot_wireframe(o_center[::200,0], o_center[::200,1], o_center[::200,2], color='g')
